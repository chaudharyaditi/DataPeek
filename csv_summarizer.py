...
# local pure-python stats helper
...
def to_float_or_none(s: str):
    ...
    if s == '':
        return None
    try:
        # allow things like '1,234.5' by removing commas
    ...
    for path in candidates:
        if os.path.exists(path):
            try:
                ...
                last_err = e
    # If not found, raise a clearer error
    msg = "\n".join([
...
def main(argv: List[str]) -> int:
    ...
    lib = load_lib()
    # C function signature: void summarize(const double* data, int n, double* out_min, double* out_max, double* out_mean)
    ...
    for idx, col in enumerate(columns):
        ...
        if len(nums) == 0:
            continue  # skip non-numeric columns
        arr_type = ctypes.c_double * len(nums)
        arr = arr_type(*nums)
        out_min = ctypes.c_double()
        out_max = ctypes.c_double()
        out_mean = ctypes.c_double()
        lib.summarize(arr, ctypes.c_int(len(nums)), ctypes.byref(out_min), ctypes.byref(out_max), ctypes.byref(out_mean))
        name = headers[idx] if idx < len(headers) and headers[idx] else f"col_{idx}"
        numeric_results.append((name, out_min.value, out_max.value, out_mean.value))
